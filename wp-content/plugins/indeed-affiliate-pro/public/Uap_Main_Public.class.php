...
if (!class_exists('Uap_Main_Public')){
	class Uap_Main_Public{
		...
		public function __construct(){
			...
			add_shortcode('uap-account-header', array($this, 'affiliate_print_account_header'));
			add_shortcode('uap-account-content', array($this, 'affiliate_print_account_content'));
		...
		public function affiliate_print_account_header($args=array()){
			/*
			 * @param array
			 * @return string
			 */
			$output = '';
			if ($this->is_admin && !$this->affiliate_id){
				$output = $this->return_admin_info_message('account_header');
			} else if ($this->affiliate_id){
				/// ONLY FOR AFFILIATES
				require UAP_PATH . 'public/Affiliate_Account_Page.class2.php';
				$obj = new Affiliate_Account_Page($this->user_id, $this->affiliate_id);
		...
		public function affiliate_print_account_content($args=array()){
			/*
			 * @param array
			 * @return string
			 */
			$output = '';
			if ($this->is_admin && !$this->affiliate_id){
				$output = $this->return_admin_info_message('account_content');
			} else if ($this->affiliate_id){
				/// ONLY FOR AFFILIATES
				require UAP_PATH . 'public/Affiliate_Account_Page.class3.php';
				$obj = new Affiliate_Account_Page($this->user_id, $this->affiliate_id);
				$output = $obj->output();
			}
			return $output;
